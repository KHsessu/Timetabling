...
void TEnvironment::Define()
{
  ...
  tIndi.Define( tEvaluator->fNumOfEvent ,tEvaluator->fNumOfTime , tEvaluator->fNumOfRoom);
}
...
void TEnvironment::Result( int numOfTrial, char* dstFile )
{
  ...
  sprintf( filename, "./result/%s_Timetable", dstFile );
  fp = fopen( filename, "a" );
  fprintf(fp,"r\\t ");
  ...
  for( int r = 0; r < tSearch->fNumOfRoom; ++r ){
    fprintf(fp,"%3d ",r);
    for( int t = 0; t < tSearch->fNumOfTime; ++t ){
      fprintf(fp,"%3d ",tIndi.fEvent_TimeRoom[ t ][ r ]);
    }
    fprintf(fp,"\n");
  }
  fprintf(fp,"\n");
  ...
  sprintf( filename, "./result/%s_%d_Sol", dstFile , numOfTrial );
  fp = fopen( filename, "a" );
  tIndi.WriteTo( fp );
  fclose(fp);
}
...
void TEnvironment::DoIt()
{
  ...
  */
  ...
  printf( "S1 = %d S2 = %d S3 = %d S4 = %d S5 = %d S6 = %d\n",  tSearch->fPenalty_S1,  tSearch->fPenalty_S2, tSearch->fPenalty_S3, tSearch->fPenalty_S4, tSearch->fPenalty_S5, tSearch->fPenalty_S6);
